...
class VGG(nn.Module):
    def __init__(self, features=nn.Module, num_classes=200, n_attributes=0, bottleneck=False, expand_dim=0, connect_CY=False, init_weights=True, dropout=0.5):
        """
        Args:
        ...
        if self.n_attributes > 0:
            if not bottleneck: #multitasking
                self.all_fc.append(FC(4096, num_classes, expand_dim))
            for i in range(self.n_attributes):
                self.all_fc.append(FC(4096, 1, expand_dim))
        else:
            self.all_fc.append(FC(4096, num_classes, expand_dim))
