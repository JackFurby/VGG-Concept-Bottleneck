...
class VGG(nn.Module):
    def __init__(self, features=nn.Module, num_classes=200, n_attributes=0, bottleneck=False, expand_dim=0, connect_CY=False, init_weights=True, dropout=0.5, all_fc=nn.ModuleList()):
        """
        Args:
        ...
        # Note: this only works if expand_dim==0 (try to update with an expand_dim value)
        temp_weights = []
        temp_biases = []

        for i in all_fc:
            temp_weights.append(i.fc.weight)
            temp_biases.append(i.fc.bias)

        temp_weights = torch.cat(temp_weights, dim=0)
        temp_biases = torch.cat(temp_biases, dim=0)

        self.all_fc_linear = nn.Linear(4096, n_attributes)

        self.all_fc_linear.weight = nn.Parameter(temp_weights)
        self.all_fc_linear.bias = nn.Parameter(temp_biases)
